...
def shape_broadcast(shape1, shape2):
    ...
    for i in range(m):
        if i >= len(shape1):
            result[i] = shape2_rev[i]

        elif i >= len(shape2):
            result[i] = shape1_rev[i]

        else:
            result[i] = max(shape1_rev[i], shape2_rev[i])

            if shape1_rev[i] != result[i] and shape1_rev[i] != 1:
                raise IndexingError("Failed to broadcast {shape1} {shape2}")

            if shape2_rev[i] != result[i] and shape2_rev[i] != 1:
                raise IndexingError("Failed to broadcast {shape1} {shape2}")
