...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        for idx, var in enumerate(inputs):
            ...
            elif type(derivatives) == float or type(derivatives) == int:
                var_list.append(tuple((var, derivatives)))

            else:
                var_list.append(tuple((var, derivatives[idx])))
...
def backpropagate(variable, deriv):
    ...
    while not q2.empty():
        ...
        if var.is_leaf():
            ...
            vars_with_derivs = history.backprop_step(der)

            for v, k in vars_with_derivs:
