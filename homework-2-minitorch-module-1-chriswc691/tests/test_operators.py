...
def test_sigmoid(a):
    ...
    * it is  strictly increasing.
    ...
    if a >= 0:
        assert sigmoid(a) == 1 / (1 + math.exp(-a))
    if a < 0:
        assert sigmoid(a) == math.exp(a) / (1 + math.exp(a))
...
def test_distribute(a, b, c):
    ...
    # TODO: Implement for Task 0.2.
    # raise NotImplementedError('Need to implement for Task 0.2')
    assert_close(mul(c, (a + b)), c * a + c * b)
    # assert mul(a, (b+c)) == a*b + a*c
...
def test_sum_distribute(ls1, ls2):
    ...
    x = 0
    for element in ls1:
        ls3.append(element + ls2[x])
        x += 1

    assert_close(sum(ls1) + sum(ls2), sum(ls3))
